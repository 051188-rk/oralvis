...
export default function AnnotationCanvas({ submission, onSaved }){
  ...
  const drawAll = (ctx, list, img)=>{
    ...
    list.forEach(s=>{
      if(s.type==='pen'){
        ...
        ctx.strokeStyle = s.color || 'red'; ctx.lineWidth = s.width||3; ctx.stroke();
      }else if(s.type==='rect'){
        ctx.strokeStyle = s.color || 'lime'; ctx.lineWidth = s.width||2;
  ...
  const save = async ()=>{
    ...
    try{
      const res = await authAxios.put('/api/submissions/' + submission._id + '/annotate', { annotationData: { strokes }, annotatedImage: dataUrl });
  ...
  return (
    <div className='card'>
      ...
      <div className='controls'>
        <button className='btn' onClick={()=>setMode('pen')}>Pen</button>
        <button className='btn' onClick={()=>setMode('rect')}>Rect</button>
        <button className='btn' onClick={clearAll}>Clear</button>
        <button className='btn primary' onClick={save}>Save Annotation</button>
      </div>
