...
export default function PatientForm(){
  ...
  const [form, setForm] = useState({ patientName:'', patientID:'', email:'', note:'', upper:null, front:null, lower:null });
  const [mine, setMine] = useState([]);

  useEffect(()=>{ if(user) setForm(f=>({...f, patientName: user.name, email: user.email})); loadMine(); }, []);

  const loadMine = async ()=>{ try{ const res = await authAxios.get('/api/submissions/mine'); setMine(res.data); }catch(e){ console.log(e); } };
  ...
  const submit = async (e)=>{
    e.preventDefault();
    if(!form.upper || !form.front || !form.lower){ alert('Please upload all three images'); return; }
